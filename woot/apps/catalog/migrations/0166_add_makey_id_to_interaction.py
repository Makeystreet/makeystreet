...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for interaction in orm.UserInteraction.objects.all():
            if Interaction.from_id(interaction.event).name == "activity_insight_created":
                try:
                    insight = orm.Note.objects.get(id=interaction.event_id)
                    if insight.makeynotes.count() == 1:
                        makey = insight.makeynotes.all()[0]
                        interaction.makey = makey
                        interaction.save()
                except orm.Note.DoesNotExist:
                    interaction.delete()

            elif Interaction.from_id(interaction.event).name == "activity_question_created":
                try:
                    question = orm.Question.objects.get(id=interaction.event_id)
            ...
            elif Interaction.from_id(interaction.event).name == "activity_answer_created":
                try:
                    answer = orm.Answer.objects.get(id=interaction.event_id)
