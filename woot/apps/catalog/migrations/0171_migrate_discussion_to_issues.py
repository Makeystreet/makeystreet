...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for question in orm.Question.objects.all():
            if not question.makey:
                continue
            issue = orm.Issue()
            issue.makey = question.makey
            issue.owner = question.creator
            issue.title = question.name
            issue.save()
            issue.added_time = question.created
            issue.save()

            #Add the question description as a comment
            if question.description:
                comment = orm.Comment()
                comment.user = question.creator
                comment.body = question.description
                comment.save()
                comment.added_time = question.created
                comment.save()
                issue.comments.add(comment)

            #transfer question comments to issue comments
            for question_comment in question.comments.all():
                if question_comment.body:
                    comment = orm.Comment()
                    comment.user = question_comment.user
                    comment.body = question_comment.body
                    comment.save()
                    comment.added_time = question_comment.added_time
                    comment.save()
                    issue.comments.add(comment)

            #Add answers as comment
            for answer in question.answer_set.all():
                comment = orm.Comment()
                comment.user = answer.creator
                comment.body = answer.description
                comment.save()
                comment.added_time = answer.created
                comment.save()
                issue.comments.add(comment)

                #transfer question comments to issue comments
                for answer_comment in answer.comments.all():
                    if answer_comment.body:
                        comment = orm.Comment()
                        comment.user = answer_comment.user
                        comment.body = answer_comment.body
                        comment.save()
                        comment.added_time = answer_comment.added_time
                        comment.save()
                        issue.comments.add(comment)
    ...
    models = {
        ...
        'catalog.issue': {
            ...
            'milestone': ('django.db.models.fields.related.ManyToManyField', [], {'blank': 'True', 'related_name': "'issues'", 'null': 'True', 'symmetrical': 'False', 'to': "orm['catalog.Milestone']"}),
            ...
            'subscribers': ('django.db.models.fields.related.ManyToManyField', [], {'symmetrical': 'False', 'to': u"orm['auth.User']", 'null': 'True', 'blank': 'True'}),
